...
/ip ipsec profile add dh-group=ecp256 prf-algorithm=sha256 nat-traversal=yes proposal-check=obey dpd-interval=10s dpd-maximum-failures=1 name "IKE2-PROFILE"
...
/ip ipsec policy add src-address=${PAIR.remote} dst-address=${PAIR.yc} tunnel=yes action=encrypt proposal=IKE2-PROPOSAL peer=YC
/ip firewall nat add chain=srcnat src-address=${PAIR.remote} dst-address=${PAIR.yc} action=accept
%{ endfor ~}
